...
app.post('/registration', jsonParser, (req, res) => {
    const promise = new Promise((resolve, reject) => {
        connection.execute(
            'SELECT COUNT(name) FROM user WHERE name = ?',
            [req.body.name],
            function (err, row) {
                console.log(row);
                if (err) {
                    ...
                    for (let i = 1; i <= row.length; i++) {
                        connection.execute(
                            ...
                            function (err, rows) {
                                console.log("ROWS!~!!!@!!@!@!");
                                console.log(rows);
                                spisok_cart.push(rows[0]);
                                console.log("SPIPSPSIPAISPISPIPS");
                                console.log(spisok_cart);
                                if (row.length == i && spisok_cart.length !== 0) {
                                    if (spisok_cart[0].length == 0) {
                                        res.send(204);
                                    } else {
                                        res.send(spisok_cart);
                                    }
                                }
                            }
                        );
